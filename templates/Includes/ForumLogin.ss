<div id="RegisterLogin">
	<% if CurrentMember %>
		<p>
			<%t ForumLogin_ss.LOGGEDINAS %> <% if CurrentMember.Nickname %>$CurrentMember.Nickname<% else %><%t ForumLogin_ss.ANONYMOUS "Anonymous" %><% end_if %> |
			<a href="$ForumHolder.Link(logout)" title="<%t ForumLogin_ss.LOGOUTEXPLICATION "Click here to log out" %>"><%t ForumLogin_ss.LOGOUT "Log Out" %></a> |
			<a href="ForumMemberProfile/edit" title="<%t ForumLogin_ss.PROFILEEXPLICATION "Click here to edit your profile" %>"><%t ForumLogin_ss.PROFILE "Profile" %></a>
		</p>
	<% else %>
		<p>
